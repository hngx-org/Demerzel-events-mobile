...
class PaginationNotifier<Group> extends StateNotifier<PaginationState<Group>> {
  PaginationNotifier({
    required this.fetchNextItems,
    required this.itemsPerBatch,
  }) : super(const PaginationState.loading());

  final Future<List<Group>> Function(
    int? page,
  ) fetchNextItems;
  final int itemsPerBatch;
  int page = 1;
  final List<Group> _items = [];
  ...
  void updateData(List<Group> result) {
...
log(page.toString());
  ...
  Future<void> fetchFirstBatch() async {
    try {
      ...
      final List<Group> result = _items.isEmpty
  ...
  Future<void> fetchNextBatch() async {
    ...
    if (state == PaginationState<Group>.onGoingLoading(_items)) {
      log("Rejected");
      return;
    }
